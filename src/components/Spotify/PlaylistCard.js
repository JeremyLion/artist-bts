...
const PlaylistCard = ({data}) => {
  return <>
   { data.map((item, key) => {
      { return item.tracks.total !== 0 ?
         <div className="playlist flex items-top justify-center bg-gray-900" key={key}>
          <Link to={`/playlist/${item.id}`} className="playlist__wrapper hover:bg-gray-700 delay-50 duration-100 bg-gray-800 p-5 rounded-lg w-60 group">
            { item?.images[0]?.url ?
              <img src={item?.images[0]?.url} className="w-full rounded shadow" alt={ item.name }/> : 
            ...
            <p className="text-gray-400 font-light mt-2 text-xs">{ item.description }</p>
          </Link>
        </div>
       : null }}
    )}
